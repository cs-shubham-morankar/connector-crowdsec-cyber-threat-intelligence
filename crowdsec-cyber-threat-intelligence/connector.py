...
class CrowdSec(Connector):
    def execute(self, config, operation, params, **kwargs):
        """
        Execute the specified operation with given configuration and parameters

        Args:
            config (dict): Connector configuration including API keys and URLs
    ...
    def check_health(self, config):
        ...
        try:
            ...
            logger.info("Starting health check for CrowdSec connector")
    ...
    def __str__(self):
        ...
        return f"CrowdSec Connector v{self._info_json.get('version', 'unknown')}"

    def __repr__(self):
        """Detailed string representation for debugging"""
        return (
            f"CrowdSec(name='{self._info_json.get('name')}', "
